class Private::ClockInController < ApplicationController
  ...
  def new
    ...
    @not_clocked_in = TimeEntry.find(:all, :conditions => {:clock_out => nil, :active => nil})
  ...
  def create
    ...
    params[:users].each do |attributes|
      @entry = TimeEntry.find(:first, :conditions => {:user_id => attributes[:user_id], :active => nil})
      unless @entry.blank?
...
private
  ...
  def build_clocked_in_ids
    @clocked_in = TimeEntry.find(:all, :conditions => {:time_sheet_id => nil, :active => true})
    @clocked_in_ids = []
