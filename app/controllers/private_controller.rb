  ...
  def navigate
    if params[:navigate] && params[:job] && params[:job][:id]
      ...
      case params[:navigate]
        ...
        when "new_time_sheet" then @redirect = url_for new_private_time_sheet_path(:job_id => @job.id)
        when "new_load_sheet" then @redirect = url_for new_private_load_sheet_path(:job_id => @job.id)
